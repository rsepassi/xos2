...
pub const GlyphLocs = struct {
    ...
    pub fn init(gfx: appgpu.Gfx, allocator: std.mem.Allocator, nmax: usize) !@This() {
        const nvertices = nmax * vertices_per_glyph;

        const vertices = try allocator.alloc(Vertex, nvertices);
    ...
    pub fn deinit(self: @This()) void {
        defer self.allocator.free(self.vertices);
    ...
    pub fn add(self: *@This(), locs: []const Loc) !void {
        const nvertices = locs.len * vertices_per_glyph;
        if ((self.nvertices + nvertices) > (self.nmax * vertices_per_glyph)) return error.TooManyVertices;
        ...
        const offset = @sizeOf(Vertex) * self.nvertices;
        self.gfx.queue.writeBuffer(self.pos, offset, self.vertices[0..nvertices]);
...
const GlyphPipelineArgs = struct {
...
pub const Args = GlyphPipelineArgs;
