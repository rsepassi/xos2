...
KHASH_MAP_INIT_INT(mClayText, Clay_Dimensions);
...
Clay_Dimensions Clay__MeasureTextCached(Clay_String *text, Clay_TextElementConfig *config) {
  static khash_t(mClayText)* txtcache = NULL;
  if (txtcache == NULL) txtcache = kh_init(mClayText);

  uint32_t id = Clay__HashTextWithConfig(text, config);
  khiter_t iter = kh_get(mClayText, txtcache, id);
  if (iter != kh_end(txtcache)) return kh_val(txtcache, iter);

  Clay_Dimensions measured = Clay__MeasureText(text, config);
  int ret;
  khiter_t key = kh_put(mClayText, txtcache, id, &ret);
  kh_val(txtcache, key) = measured;
  return measured;
}
