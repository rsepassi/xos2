...
# Based on https://github.com/create-dmg/create-dmg
# License: MIT
...
SIGNATURE=""
NOTARIZE=""
LINK_NAME="Drag here to install"
...
while [[ "${1:0:1}" = "-" ]]; do
	case $1 in
		...
		--app-drop-link)
			APPLICATION_LINK=$2
			APPLICATION_CLAUSE="set position of item \"$LINK_NAME\" to {$2, $3}
			"
			shift; shift; shift;;
		--format)
			FORMAT="$2"
...
if [[ -n "$APPLICATION_LINK" ]]; then
	...
	echo $MOUNT_DIR
	ln -s /Applications "$MOUNT_DIR/$LINK_NAME"
fi
